...
if not(os.name is "posix"):
    ...
    class workPage(GridLayout):
        def __init__(self, **kwargs):
            ...
            self.add_widget(Button(text='All Work macros ',on_press = callback, on_release = callback,background_color = (1,1,1,1)))
...
def callback8(self):
    ...
    subprocess.call('~/.local/bin/autokey-run -s copy', shell = True)
...
def workMain(self):
    subprocess.call(['python3',''])
def gamingMainPosix(self):
    subprocess.call('cd ~', shell = True)
    subprocess.call('~/.local/bin/autokey-run -s launcher', shell = True)
def workMainPosix(self):
    subprocess.call('cd ~', shell = True)
    subprocess.call('~/.local/bin/autokey-run -s launcher', shell = True)
def customMain(self):
    subprocess.call(['python3',''])
